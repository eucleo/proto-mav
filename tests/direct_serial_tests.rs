...
mod test_direct_serial {
    ...
    pub fn test_incomplete_address() {
        let conn_result = proto_mav::connect::<MavMessage>("serial:");
        assert!(conn_result.is_err(), "Incomplete address should error");
    }

    #[test]
    pub fn test_bogus_baud() {
        let conn_result = proto_mav::connect::<MavMessage>("serial:port1:badbaud");
    ...
    pub fn test_nonexistent_port() {
        ...
        let conn_result = proto_mav::connect::<MavMessage>(bogus_port_str);
